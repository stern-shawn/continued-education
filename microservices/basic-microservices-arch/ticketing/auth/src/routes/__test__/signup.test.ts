...
it('returns a 400 for invalid email', async () => {
  return request(app).post('/api/users/signup').send({ email: 'nowayman', password: 'password' }).expect(400);
});
...
it('returns a 400 for missing email and password', async () => {
  ...
  return request(app).post('/api/users/signup').send({ password: 'p' }).expect(400);
});
